...
├── iocs.csv                        244행 (도메인 190)
└── traffic_summary.txt             약 2,540 세션 (payload 보유 177)
```
...
```
ATT&CK 표의 6개 ID 중 3개를 고르는 조합 수백 개 × payload 177개 ≈ 2만 회
```
...
| `SAMPLESHA` | 샌드박스 JSON **전용** — 다른 어디에도 없다 |
...
```
트래픽 도메인 ∩ IOC 지표      초안 1개  ->  현재 48개
샌드박스 도메인 ∩ IOC 지표    초안 1개  ->  현재 7/7
```

**두 번째 줄이 4차에서 새로 잡힌 것이다.** 트래픽 쪽 교집합은 막았는데,
샌드박스 3종에 등장하는 도메인 7개 중 IOC 목록에 있는 것이 C2 하나뿐이었다.
`comm` 한 번이면 C2·first_seen·유출 세션·wave3 샘플이 한꺼번에 나온다.
같은 구멍이 죽은 게 아니라 옮겨간 것이었다.

### IOC 잡음 도메인이 미끼처럼 생기면 안 된다

초안은 잡음 150개 중 144개가 `api-1a2b3.example.com` 형태의 생성기 템플릿이라,
**템플릿에 안 맞는 6개**(C2 와 유사 도메인)를 정규식 한 줄로 뽑을 수 있었다.

```
^[a-z]+-[0-9a-f]{5}\.example\.(com|net|org)$   에 안 맞는 도메인
초안: 6개      현재: 190/190
```

사람이 지을 법한 이름(`cdn-sync`, `ocsp-r3`, `mirror-eu` …)으로 바꿨다.

### 타임스탬프 복제가 오히려 정답을 표시했다

숨기려고 `first_seen` 을 몇 개 복제했는데, 날짜만 흔들고 시:분:초를 재사용해서
`09:21:44` 가 **59번** 등장했다. 장치가 정확히 반대로 작동한 것이다.
초 단위까지 완전 무작위로 바꿔 현재 최다 중복 1회.
